...
import { FLUSH, PAUSE, PERSIST, PURGE, REHYDRATE, DEFAULT_VERSION, RESYNC } from './constants';
    ...
    return (state, action) => {
        ...
        if (action.type === PERSIST) {
            ...
            getStoredState(config)
                .then(restoredState => action.rehydrate(config.key, restoredState, undefined), err => action.rehydrate(config.key, undefined, err))
                .then(() => action.result());
            return Object.assign(Object.assign({}, baseReducer(restState, action)), { _persist: { version, rehydrated: false } });
        }
